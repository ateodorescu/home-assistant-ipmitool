...
class IpmiConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle a config flow for ipmitool."""

    VERSION = 1

    def __init__(self) -> None:
        """Initialize the ipmitool config flow."""
    ...
    async def async_step_zeroconf(
        ...
        """Prepare configuration for a discovered ipmitool device."""
    ...
    def _host_port_alias_already_configured(self, user_input: dict[str, Any]) -> bool:
        """See if we already have an ipmitool entry matching user input configured."""
...
class OptionsFlowHandler(OptionsFlow):
    """Handle an option flow for ipmitool."""
